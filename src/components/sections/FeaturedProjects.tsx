...
export default function FeaturedProjects() {
  const featured = realisations.filter((r) => r.featured).slice(0, 3);
            ...
            <Link href={`/realisations/${main.slug}`} className="group relative block overflow-hidden rounded-2xl aspect-[4/3]">
              ...
              <div className="absolute bottom-0 left-0 p-6">
                <p className="text-xs font-semibold text-accent-400 uppercase tracking-wider mb-1">
                  {REALISATION_CATEGORY_LABELS[main.category]}
                </p>
                <h3 className="font-heading text-xl font-bold text-white">{main.title}</h3>
                <p className="text-sm text-neutral-300 mt-1">{main.origin} → {main.destination}</p>
              </div>
            </Link>
          </FadeUp>

          {/* Two smaller realisations */}
          <div className="grid grid-rows-2 gap-4">
            {rest.map((realisation, idx) => (
              <FadeUp key={realisation.id} delay={(idx + 1) * 0.1}>
                <Link href={`/realisations/${realisation.slug}`} className="group relative block overflow-hidden rounded-2xl h-full min-h-[180px]">
                  <Image
                  ...
                  <div className="absolute bottom-0 left-0 p-5">
                    ...
                    <h3 className="font-heading text-lg font-bold text-white">{realisation.title}</h3>
                  </div>
                </Link>
              </FadeUp>
            ))}
