  ...
  return (
    <header
      className={cn(
        ...
        transparent
          ? "bg-transparent"
          : "bg-white/95 backdrop-blur-sm shadow-[0_1px_0_0_oklch(0.89_0.015_255)]"
      )}
    >
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="flex h-16 md:h-20 items-center justify-between gap-8">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/contact"
              className="hidden md:inline-flex items-center px-5 py-2.5 rounded-xl text-sm font-semibold bg-accent-500 text-white hover:bg-accent-600 hover:shadow-lg hover:shadow-accent-500/20 hover:-translate-y-0.5 transition-all duration-300"
            >
              Devis Rapide
            </Link>
          ...
          >
            <div className="px-6 py-4 space-y-1">
                    ...
                    <div className="ml-3 mt-1 border-l-2 border-neutral-100 pl-3 space-y-1">
                      {services.map((svc) => (
                        <Link
                          key={svc.id}
                          href={`/services/${svc.slug}`}
                          className="block py-2 text-sm text-neutral-600 hover:text-primary-800 transition-colors"
                        >
                          {svc.title}
                        </Link>
                      ))}
              ...
              <div className="pt-3 border-t border-neutral-100">
                <Link
                  href="/contact"
                  className="block w-full text-center px-4 py-3 rounded-xl text-sm font-semibold bg-accent-500 text-white hover:bg-accent-600 transition-colors"
                >
